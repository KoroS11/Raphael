...
export function AorMap({
  ...
  const debugGrid = useDebugStore((s) => s.gridVisible)
  const debugImagery = useDebugStore((s) => s.imageryVisible)
  const debugMuted = useDebugStore((s) => s.mutedVisible)
  const setTileStats = useDebugStore((s) => s.setTileStats)
  ...
  // Debug toggles: override layer visibility and surface tile-load activity.
  useEffect(() => {
    const map = mapRef.current
    if (!map || !isMapReady) return
    const gridVisible = debugEnabled ? debugGrid : true
    map.setLayoutProperty(
      'mgrs-grid-line',
      'visibility',
    ...
    if (debugEnabled) {
      map.setLayoutProperty(
        'imagery',
        'visibility',
        debugImagery ? 'visible' : 'none',
      )
      map.setLayoutProperty(
        'muted',
        'visibility',
        debugMuted ? 'visible' : 'none',
      )
    }
  }, [debugEnabled, debugGrid, debugImagery, debugMuted, isMapReady, basemap])
  ...
  useEffect(() => {
    ...
    const inc = () => {
      pending += 1
      setTileStats(pending, false)
    }
    const dec = () => {
      pending = Math.max(0, pending - 1)
      setTileStats(pending, map.areTilesLoaded())
    }
    const onIdle = () => setTileStats(pending, true)
