...
def _make_id():
    """UUID as string for SQLite, native UUID for Postgres."""
    return str(uuid.uuid4()) if IS_SPATIALITE else uuid.uuid4()
...
def seed_demo(fresh: bool = False):
    ...
    try:
        ...
        pune_id = "74e952c3-29f3-48f2-a71e-8f5725ede8e1"
        delhi_id = _make_id()
        ...
        if not existing_pune:
            ...
            pune_id = str(existing_pune.id) if IS_SPATIALITE else existing_pune.id
        ...
        existing_zone_count = db.query(ZoneGeometry).filter(
            ZoneGeometry.region_id == str(pune_id) if IS_SPATIALITE else ZoneGeometry.region_id == pune_id
        ).count()
